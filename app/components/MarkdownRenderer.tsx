...
interface MarkdownRendererProps {
    ...
    forceShowOpenCanvasButton?: boolean; // NEW
}
...
// helper to ensure arrays of children have keys
const addKeys = (children: React.ReactNode, prefix: string) =>
    Children.map(children, (child, idx) => (isValidElement(child) ? cloneElement(child, { key: `${prefix}-${idx}` }) : child));
...
export default function MarkdownRenderer({
    ...
    isCanvasMode,
    onOpenCodeCanvas,
    forceShowOpenCanvasButton
}: MarkdownRendererProps) {
    const components: Components = {
        p: ({ children }) => <div className="mb-4">{addKeys(children, 'p')}</div>,
                    ...
                    return (
                        <div className="my-4 rounded-md bg-blue-50 border border-blue-200 p-4 font-sans shadow-sm">
                            <div className="flex items-center justify-between">
                                ...
                                {(block.isComplete || forceShowOpenCanvasButton) && (
                                    <button
        ...
        th({ children }) {
            return (
                <th className="border border-border-color bg-input-bg p-2 text-left font-semibold">
                    {addKeys(children, 'th')}
                </th>
            );
        },
        ...
        td({ children }) {
            return <td className="border border-border-color p-2">{addKeys(children, 'td')}</td>;
        }
    };

    return (
        <div className="prose prose-sm max-w-none text-primary-text">
            <ReactMarkdown remarkPlugins={[remarkGfm]} components={components} unwrapDisallowed={true}>
                {content}
            </ReactMarkdown>
        </div>
    );

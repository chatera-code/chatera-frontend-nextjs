...
export default function ChatArea({ selectedSessionId, selectedDocs, clientId, statusUpdate }: ChatAreaProps) {
  ...
  const isInputDisabled = (!selectedSessionId || isStreaming);
    ...
    <div className="flex-1 flex flex-col h-screen bg-background relative">
      <header className="flex items-center justify-between h-[60px] px-6 bg-background flex-shrink-0 z-10">
        <h1 className="text-lg font-semibold text-primary-text">
          {selectedSessionId ? `Conversation Details` : 'Select or Start a Conversation'}
        </h1>
      </header>
      <div
        className={`flex-1 overflow-y-auto px-6 pt-0 ${isChatStarted ? 'pb-48' : ''} space-y-4 w-full max-w-3xl mx-auto transition-all duration-1000`}
        style={{ scrollPaddingTop: '60px' }} // Matches header height
      >
      ...
      <div className="absolute top-[60px] left-1/2 -translate-x-1/2 w-full max-w-3xl h-12 bg-gradient-to-b from-background/100 to-background/0 pointer-events-none z-10" />
      ...
      <footer className={`bg-background transition-all duration-1000 ease-in-out z-10 w-full max-w-3xl absolute left-1/2 ${isChatStarted ? 'top-full -translate-x-1/2 -translate-y-full' : 'top-1/2 -translate-x-1/2 -translate-y-1/2'}`}>
        <form onSubmit={handleSendMessage} className="relative">
          <textarea
            ...
            placeholder={!selectedSessionId ? "Start a new chat to begin..." : "Type your message..."}
